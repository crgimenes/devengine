...
import "github.com/crgimenes/devengine/utils"
...
// eav_values.
func (t *Transaction) SaveRecordValues(recordID int64, attributes []EAVAttribute, values EAVRecordValues) error {
	for machineName, rawValue := range values {
		...
		const qUpsert = `INSERT OR REPLACE INTO eav_values (
            record_id,    -- 1
            attribute_id, -- 2
            v_bool,       -- 3
            v_int,        -- 4
