...
func (s *SQLite) CreateAPIToken(userID int64, tokenHash, label string) (*db.APIToken, error) {
	if tokenHash == "" {
...
func (s *SQLite) ListAPITokensByUserID(userID int64) ([]db.APIToken, error) {
	const q = `SELECT
        id,         -- 1
        user_id,    -- 2
        label,      -- 3
...
func (s *SQLite) DeleteAPIToken(id, userID int64) error {
...
func (s *SQLite) GetUserByAPITokenHash(tokenHash string) (*db.User, error) {
	const q = `SELECT ` + userSelectColumns + `
    FROM users
    WHERE id = (SELECT user_id FROM api_tokens WHERE token_hash = ?) -- 1
    AND enabled = 1`
