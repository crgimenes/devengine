...
The `sqlite` package implements the `db.Store` contract with an opinionated SQLite layer built on top of the `modernc.org/sqlite` driver (CGO free). It gives the application safe defaults for concurrency, durability, and observability without forcing extra dependencies.
...
```go
import "github.com/crgimenes/devengine/db/sqlite"
```
...
Use `sqlite.New()` when you want to respect the configured database path (from `config.Cfg.DBFile`). If no path is configured it falls back to `devengine.db` in the working directory.
...
```go
tmpDir := t.TempDir()
store, err := sqlite.NewWithPath(filepath.Join(tmpDir, "test.db"))
if err != nil {
...
```go
func shutdown(ctx context.Context, store *sqlite.SQLite) {
