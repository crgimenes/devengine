...
// ListFilesByMediaType returns files for a user filtered by media type (image, video, audio)
// with pagination. The mediaType parameter is matched against the MIME type prefix.
func (s *SQLite) ListFilesByMediaType(
	userID int64,
	mediaType string,
	offset int,
	limit int,
) ([]*File, error) {
	// Build LIKE pattern for media type
	...
	if err != nil {
		...
		log.Println("ListFilesByMediaType query error:", err)
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			...
			log.Println("ListFilesByMediaType scan error:", err)
			return nil, err
		}
		files = append(files, &f)
	}
...
// CountFilesByMediaType returns the total count of files for a user filtered by media type.
func (s *SQLite) CountFilesByMediaType(
	userID int64,
	mediaType string,
) (int, error) {
	var likePattern string
	...
	var count int
	err := s.QueryRow(sqlCount, userID, likePattern).Scan(&count)
	if err != nil {
		log.Println("CountFilesByMediaType error:", err)
		return 0, err
	}
