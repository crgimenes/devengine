...
func TestUpdateUserProfileWithoutEmailDoesNotRequireOne(t *testing.T) {
	...
	// User created with no email — the bug we're fixing rejected this on save.
...
func TestIsValidUsername(t *testing.T) {
	...
	for _, s := range good {
		if err := IsValidUsername(s); err != nil {
			t.Errorf("IsValidUsername(%q) = %v, want nil", s, err)
		}
	}
	...
	for _, s := range bad {
		if err := IsValidUsername(s); err == nil {
			t.Errorf("IsValidUsername(%q) accepted invalid username", s)
		}
	}
}
