...
func TestEAVFullFlow(t *testing.T) {
	...
	// 2. Form
	f, err := s.CreateEAVForm(w.ID, 1, "test-form", "Test Form", "list")
	if err != nil {
	...
	if fields[0].ColumnWidth != 6 {
		t.Errorf("expected first field width 6, got %d", fields[0].ColumnWidth)
	}
	if fields[1].ColumnWidth != 12 {
		t.Errorf("expected second field width 12, got %d", fields[1].ColumnWidth)
	}
...
func TestGetEAVFieldsByFormIDOrdering(t *testing.T) {
	...
	form, err := s.CreateEAVForm(ws.ID, 1, "ordering-form", "Ordering Form", "list")
	if err != nil {
		t.Fatalf("CreateEAVForm error: %v", err)
	}
...
func TestGetEAVFieldsByProcessingOrder(t *testing.T) {
	...
	form, err := s.CreateEAVForm(ws.ID, 1, "proc-form", "Proc Form", "list")
	if err != nil {
		t.Fatalf("CreateEAVForm error: %v", err)
	}
...
func TestEAVFieldGroupingFlagPersistence(t *testing.T) {
	t.Parallel()
	...
	const sqlInsertUser = `INSERT INTO users (username, email, enabled) VALUES (?, ?, ?)`
	if err := s.Exec(sqlInsertUser, "group_owner", "group_owner@example.com", 1); err != nil {
		t.Fatalf("insert user: %v", err)
	}

	ws, err := s.CreateEAVWorkspace("Workspace Grouping", "")
	if err != nil {
		t.Fatalf("CreateEAVWorkspace error: %v", err)
	}

	form, err := s.CreateEAVForm(ws.ID, 1, "group-form", "Group Form", "list")
	if err != nil {
		t.Fatalf("CreateEAVForm error: %v", err)
	}
	...
	if !field.IsGroupingField {
		t.Fatalf("expected grouping flag to persist on create")
	}

	updated, err := s.UpdateEAVField(field.ID, field.Label, field.ZOrder, field.ColumnWidth, field.ParentGroupMachineName, false, field.IsUI, field.PrimitiveKind, field.UIKind, field.UIMetaJSON, field.Expression, field.ExpressionOrder, field.IsReadonly, field.Required, true, 0, 12, true, 0, 12, true, 0, 12, false)
	if err != nil {
		t.Fatalf("UpdateEAVField grouping: %v", err)
	}

	if updated.IsGroupingField {
		t.Fatalf("expected grouping flag to update to false")
	}
}
...
func TestSoftDeleteEAVField(t *testing.T) {
	...
	form, err := s.CreateEAVForm(ws.ID, 1, "field-delete-form", "Field Delete Form", "list")
	if err != nil {
		t.Fatalf("CreateEAVForm error: %v", err)
	}
...
func TestGetEAVFormByMachineName(t *testing.T) {
	t.Parallel()
	...
	ws, err := s.CreateEAVWorkspace("Workspace Machine Name", "")
	if err != nil {
		t.Fatalf("CreateEAVWorkspace error: %v", err)
	}

	created, err := s.CreateEAVForm(ws.ID, 1, "runtime-form", "Runtime Form", "list")
	if err != nil {
		t.Fatalf("CreateEAVForm error: %v", err)
	}

	got, err := s.GetEAVFormByMachineName(ws.ID, "runtime-form")
	if err != nil {
		t.Fatalf("GetEAVFormByMachineName error: %v", err)
	}
...
func TestSoftDeleteRecordAndValueRemoval(t *testing.T) {
	...
	form, err := s.CreateEAVForm(ws.ID, 1, "delete-form", "Delete Form", "list")
	if err != nil {
		t.Fatalf("CreateEAVForm error: %v", err)
	}
