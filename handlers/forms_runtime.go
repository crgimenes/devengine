...
func (h *Handlers) FormsRuntimeNew(w http.ResponseWriter, r *http.Request) {
	...
	h.renderRuntimeForm(w, r, user, ctx, nil, values, nil, message, errorMsg, posLoadError)
}
...
func (h *Handlers) FormsRuntimeCreate(w http.ResponseWriter, r *http.Request) {
	...
	if len(fieldErrors) > 0 {
		h.renderRuntimeForm(w, r, user, ctx, nil, parsedValues, fieldErrors, "", "", "")
		return
	}
	...
	if err != nil {
		h.renderRuntimeForm(w, r, user, ctx, nil, parsedValues, nil, "", tr(r, "Could not start the transaction"), "")
		return
	}
	committed := false
	defer func() {
	...
	if err != nil {
		h.renderRuntimeForm(w, r, user, ctx, nil, parsedValues, nil, "", saveErrorMessage(r, "insertRecordTx", err), "")
		return
	}
	...
	if err != nil {
		...
		h.renderRuntimeForm(w, r, user, ctx, nil, parsedValues, nil, "", tr(r, "Could not finish saving (ref %s)", ref), "")
		return
	}
	committed = true

	http.Redirect(w, r, "/form/"+machineName+"/r/"+recordRefID+"?message="+tr(r, "Record created successfully"), http.StatusSeeOther)
}
...
func (h *Handlers) FormsRuntimeEdit(w http.ResponseWriter, r *http.Request) {
	...
	h.renderRuntimeForm(w, r, user, ctx, record, values, nil, message, errorMsg, posLoadError)
}
...
func (h *Handlers) FormsRuntimeUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if submittedRev != record.Rev {
		h.renderRuntimeForm(w, r, user, ctx, record, parsedValues, nil, "",
			tr(r, "The record was modified by another user. Review the data before saving again."), "")
		return
	}

	if len(fieldErrors) > 0 {
		h.renderRuntimeForm(w, r, user, ctx, record, parsedValues, fieldErrors, "", "", "")
		return
	}
	...
	tx, err := db.Storage.BeginTransaction()
	if err != nil {
		h.renderRuntimeForm(w, r, user, ctx, record, parsedValues, nil, "", tr(r, "Could not start the transaction"), "")
		return
	}
	committed := false
	defer func() {
	...
	if err != nil {
		h.renderRuntimeForm(w, r, user, ctx, record, parsedValues, nil, "", saveErrorMessage(r, "updateRecordTx", err), "")
		return
	}
	...
	if err != nil {
		...
		h.renderRuntimeForm(w, r, user, ctx, record, parsedValues, nil, "", tr(r, "Could not finish saving (ref %s)", ref), "")
		return
	}
	committed = true

	http.Redirect(w, r, "/form/"+machineName+"/r/"+recordRefID+"?message="+tr(r, "Record updated successfully"), http.StatusSeeOther)
}
...
func (h *Handlers) formsRuntimeButtonActionLogic(w http.ResponseWriter, r *http.Request, user *db.User, form *db.Form, button *db.FormElement, elements []db.FormElement) {
	...
	if button.ButtonRunSave {
		...
		if isUpdate {
			...
			if err != nil {
				jsonResponse(w, http.StatusBadRequest, map[string]string{"error": saveErrorMessage(r, "button insertRecordTx", err)})
				return
			}
...
func parseFormAttributes(r *http.Request, elements []db.FormElement, attributes []db.EAVAttribute) (db.EAVRecordValues, error) {
	...
	for _, el := range elements {
		...
		if rawValue == "" {
			if attr.IsRequired {
				return nil, fmt.Errorf("%s", tr(r, "required field: %s", attr.Label))
			}
...
func runPreSave(tx db.Tx, entityType *db.EAVEntityType, values db.EAVRecordValues) (db.EAVRecordValues, error) {
	...
	if execErr != nil {
		return nil, fmt.Errorf("pre_save script: %w", execErr)
	}
	if userError != "" {
		return nil, db.UserError(userError)
	}
	return modified, nil
}
...
func insertRecordTx(tx db.Tx, entityType *db.EAVEntityType, attributes []db.EAVAttribute, values db.EAVRecordValues) (int64, string, error) {
	...
	recordID, recordRefID, err := tx.InsertEAVRecord(entityType.ID)
	if err != nil {
		return 0, "", fmt.Errorf("insert record: %w", err)
	}
	err = tx.SaveRecordValues(recordID, attributes, values)
	if err != nil {
		return 0, "", fmt.Errorf("save values: %w", err)
	}
...
func updateRecordTx(tx db.Tx, entityType *db.EAVEntityType, record *db.EAVRecord, attributes []db.EAVAttribute, values db.EAVRecordValues) error {
	...
	err = tx.BumpEAVRecordRev(record.ID)
	if err != nil {
		return fmt.Errorf("bump record rev: %w", err)
	}
	err = tx.SaveRecordValues(record.ID, attributes, values)
	if err != nil {
