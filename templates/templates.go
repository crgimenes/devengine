...
// tmplFirst returns the first byte of s as a string.
func tmplFirst(s string) string {
	if len(s) == 0 {
		return ""
	}
	return string(s[0])
...
// tmplSeq generates a slice of integers from start to end (inclusive).
...
// tmplDeref dereferences a pointer to int64, returning 0 for nil.
func tmplDeref(p *int64) int64 {
	if p == nil {
		return 0
	}
	return *p
...
// tmplDict creates a map from key-value pairs for passing to templates.
...
// parseMetaWithDefaults overlays ui_meta_json onto the given defaults.
func parseMetaWithDefaults(jsonStr string, defaults map[string]any) map[string]any {
	if jsonStr == "" {
		return defaults
	}
...
// tmplSubformRecords returns the related records for a subform field.
func tmplSubformRecords(targetEntity, targetAttr, parentRef, displayAttr any) []SubformRecord {
...
// tmplGetMenuItems extracts the MenuItems field from any struct via
// reflection, returning nil when absent or empty.
func tmplGetMenuItems(data any) any {
	if data == nil {
		return nil
	}
...
// tmplGetMenuMachineName extracts the MenuMachineName field from any struct
// via reflection, returning "" when absent.
func tmplGetMenuMachineName(data any) string {
	if data == nil {
		return ""
	}
	v := reflect.ValueOf(data)
...
func templateFuncMap() template.FuncMap {
	return template.FuncMap{
		...
		"parseGroupMeta": func(jsonStr string, elementKind string) map[string]any {
			return parseMetaWithDefaults(jsonStr, getGroupDefaults(elementKind))
		},
		...
		"getMenuMachineName":  tmplGetMenuMachineName,
	}
}
