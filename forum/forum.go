...
func renderThreadCreate(w http.ResponseWriter, data threadCreateData, status int) {
	var buf bytes.Buffer

	err := templates.ExecuteTemplate(&buf, "thread_create.go.tmpl", data)
	if err != nil {
	...
	if err != nil {
		log.Printf("failed to write thread create response: %v", err)
	}
}
...
func threadViewHandler(w http.ResponseWriter, r *http.Request) {
	...
	if threadExtID == "create" {
		...
		renderThreadCreate(w, createData, http.StatusOK)
		return
	}
...
func createThreadHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = r.ParseForm()
	if err != nil {
		log.Printf("ERROR: parse form failed for new thread: %v", err)
	...
	if len(title) < 3 || len(title) > 255 {
		...
		renderThreadCreate(w, createData, http.StatusBadRequest)
		return
	}

	if len(content) < 10 {
		log.Printf("DEBUG: content validation failed - len=%d, need at least 10", len(content))
		createData := threadCreateData{
			Authed:   true,
			User:     *u,
			Config:   *config.Cfg,
